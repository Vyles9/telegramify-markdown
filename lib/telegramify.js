...
const createHandlers = (definitions, unsupportedTagsStrategy) => ({
	...
	list: (node, parent, context) => {
		const rawList = defaultHandlers.list(node, parent, context);

		if (node.ordered) {
			return rawList.replace(/^(\d+)\./gm, '$1\\.');
		} else {
			return rawList.replace(/^(\s*)\* /gm, '$1• ');
		}
	},
	...
	code(node) {
		const lang = node.lang ? node.lang.match(/^\S*/)[0] : '';
    ...
    root: (node, _parent, context) => {
        const result = defaultHandlers.root(node, _parent, context);
